...
CppDiagramGenerator::CppDiagramGenerator()
{
	InitFileTemplate();
}

void CppDiagramGenerator::InitFileTemplate()
{
...
void CppDiagramGenerator::InitVisualNodes(Circuit& circuit)
{
	...
	for(auto column : columns)
	{
		for(auto pComponent : column.components)	
		{
			...
			for(auto* pNextComponent : node.next)
			{
				...
				if(node.columnIndex + 1 == nextNode.columnIndex && node.index == nextNode.index)
				{
					...
					if(node.next.size() == 1)
					{
						...
						lineSegment.begin.x = node.position.x + NODE_SIZE / 2;
						lineSegment.begin.y = y;
						lineSegment.end.x = nextNode.position.x + NODE_SIZE / 2;
						lineSegment.end.y = y;
						lineSegment.color = "#00f";
					...
					else if(*pComponent != Input::NAME)
					{
						...
						lineSegment.begin.x = node.position.x + NODE_SIZE / 2;
						lineSegment.begin.y = y;
						lineSegment.end.x = column.edgeX;
						lineSegment.end.y = y;
						lineSegment.color = "#00f";
						lineSegments.push_back(lineSegment);
						column.edgeX += SPACE;
			...
			if(!firstLineSegment)
			{
				LineSegment lineSegment;
				lineSegment.begin.x = node.position.x + NODE_SIZE / 2;
				lineSegment.begin.y = node.position.y + NODE_SIZE / 2;
				lineSegment.end.x = column.edgeX;
				column.edgeX += SPACE;
			...
			for(auto* pNextComponent : node.next)
			{
				...
				{
					lineSegment.begin.x = vectorOrigin.x;
					lineSegment.begin.y = y;
					lineSegment.end.x = nextNode.position.x + NODE_SIZE / 2;
					lineSegment.end.y = y;
					lineSegment.color = "#00f";
				...
				else
				{
					...
					LineSegment lineSegment;
					lineSegment.begin.x = nextColumn.edgeX;		
					nextColumn.edgeX += SPACE;
					lineSegment.begin.y = vectorTop.y;		
					lineSegment.end.x = lineSegment.begin.x;
					lineSegment.end.y = y;
					lineSegments.push_back(lineSegment);

					lineSegment.begin = lineSegment.end;
					lineSegment.end.x = nextNode.position.x + NODE_SIZE / 2;
					lineSegment.color = "#00f";
					lineSegments.push_back(lineSegment);
				}
			...
			LineSegment lineSegment;
			lineSegment.begin = vectorBottom;
			lineSegment.end = vectorTop;
			lineSegments.push_back(lineSegment);

			if(hasTravelLine)
			{
				LineSegment lineSegment;
				lineSegment.begin = vectorTop;
				lineSegment.end = vectorRight;
				lineSegments.push_back(lineSegment);
			}
		}
	}
...
unsigned int CppDiagramGenerator::GetTravelY(unsigned int firstColumnIndex, unsigned int lastColumnIndex)
{
	unsigned int y = 0;
...
void CppDiagramGenerator::GenerateSvgContent()
{
	std::stringstream stream;
...
void CppDiagramGenerator::DeduceSvgSize()
{
